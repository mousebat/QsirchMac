    ...
    var body: some View {
        VStack(spacing:10) {
            ...
            HStack(alignment: .center) {
                ...
                if (networkManager.displayDrives == true){
                    ScrollView(.horizontal) {
                        Picker(selection: driveProxy, label: Text("Select Drive")) {
                            Text("All").tag("All")
                            ForEach(networkManager.DrivesListFiltered!){ drivename in
                    ...
                    Text("No Drives Available?").font(Font.system(size: 12, weight: .bold, design: .default)).foregroundColor(Color.red)
                }
                Spacer()
